...
- The PWA demo uses browser notifications subject to browser limits. The native app schedules local iOS watering reminders without a push server, but permission and delivery still need physical-device validation.
...
- Full Xcode 26.3 is selected locally. The latest integrated build, test, audit, and signing evidence is recorded below and must be rerun from the exact release commit.
...
- The pending deletion-preserving migration and matching client implement irreversible, scrubbed garden tombstones plus a server-issued account epoch, so an offline device cannot recreate a plant deleted elsewhere even with a bad clock. Once deployed in the required order, reset RPCs are idempotent by request ID, physical client deletes are revoked, and tombstones reject as well as purge watering history; account deletion remains the hard-purge path.
...
- On 2026-07-21 remote evidence confirmed the foundation migration, six RLS-protected tables, and `identify-flower` v5 are active. The historical 404s preceded the migration; unauthenticated table access and the Edge Function now fail closed with 401. The deletion-preserving migration remains intentionally pending until its client PRs are integrated.
- Authentication and garden-cloud readiness are independent: a valid Supabase login opens the local app even while the epoch migration is absent or the network is down. Garden writes require a causally observed epoch; ambiguous preflight-era and inherited reset conflicts are quarantined without blocking authorized edits, validated queue epochs survive relaunch, post-reset edits adopt the returned epoch, and sign-out clears the active account and session immediately while preserving snapshots proven to belong to another account on the shared device; remotely, it revokes only the current device's session.
- Native mutation sync waits for an authoritative readback before reporting success, and foreground activation pulls remote tombstones/resets without requiring a relaunch. Bootstrap/auth sync is cancelled before sign-out can emit a stale write, and Siri watering advances the same conflict timestamp used by cloud reconciliation. Creation provenance is scoped to the current authenticated lifecycle, so an inherited offline plant cannot be revived by a watering edit after a remote reset; queued follow-up sync reports the final reconciliation result rather than an earlier cancelled attempt.
- The beta-first-care flow now lands returning gardeners in Garden, sends first-time users to Catalog, routes a newly added flower back to Garden, exposes reminder permission only after an explicit first-care or Settings tap, and confirms the first watering immediately. Enabling notification access later in iOS Settings now reschedules the current garden before reporting reminders active. Scanner cloud consent is ephemeral and requested for every selected photo, while the explicit on-device path never uploads it. Scanner results now require an explicit review before save: provider identity remains separate from the specimen nickname, user-confirmed watering care avoids false exact precision, failed saves stay in Scanner, and successful saves clear the completed scan and open Garden. Plant deletion and local reset distinguish local completion from cloud-pending confirmation and expose an explicit retry action.
- On 2026-07-21 the configured Debug app rendered Auth and stayed alive on an iOS 26.3.1 iPhone 17 simulator; a build without the ignored local config truthfully showed the missing-publishable-key screen, while a rebuild using the existing ignored `Local.xcconfig` reached Auth without exposing or versioning the key. After integrating and race-hardening PKCE password recovery and multi-scene routing, the complete XCTest suite passed 115/115 on 2026-07-22, the unsigned Release simulator build passed, release gate passed 11/11, cloud/security audit 41/41, and native App Store audit 20/20. A Personal Team Debug build for `com.juliosuas.rocio`, team `67QTYANL3F`, installed and launched successfully on the connected iPhone after its development profile was trusted; the process remained active, confirming that the previous startup block occurred before Supabase. Its profile expires on 2026-07-28. These are dated snapshots and must be rerun from the exact release commit.
- An iPhone 16e simulator smoke on 2026-07-20 verified the demo entry, catalog, seeded garden, bundled flower photos, and local scanner disclosure. Real-device camera, photo picker, local and consented cloud analysis, scan review-to-Garden routing, and notification permission/delivery still need testing.
- Remaining App Store gaps: review and merge consolidated PR #21; allowlist `com.juliosuas.rocio://auth/recovery`; replace the localhost Auth Site URL with the chosen stable HTTPS URL; configure custom SMTP; smoke-test a real PKCE reset; deploy the deletion-preserving migration in order; run authenticated two-account/device and real-device permission smoke; activate paid distribution signing when TestFlight is next; then create the App Store Connect record, screenshots, native demo video, TestFlight upload, and final release review. The project-level `DEVELOPMENT_TEAM` is still blank.
