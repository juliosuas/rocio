...
- The beta-first-care flow now lands returning gardeners in Garden, sends first-time users to Catalog, routes a newly added flower back to Garden, exposes reminder permission only after an explicit first-care or Settings tap, and confirms the first watering immediately. Enabling notification access later in iOS Settings now reschedules the current garden before reporting reminders active. Scanner cloud consent is ephemeral and requested for every selected photo, while the explicit on-device path never uploads it. Plant deletion and local reset distinguish local completion from cloud-pending confirmation and expose an explicit retry action.
