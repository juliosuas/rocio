...
- Native mutation sync waits for an authoritative readback before reporting success, and foreground activation pulls remote tombstones/resets without requiring a relaunch. Bootstrap/auth sync is cancelled before sign-out can emit a stale write, and Siri watering advances the same conflict timestamp used by cloud reconciliation. The configured Debug app rendered Auth and stayed alive on an iOS 26.3.1 simulator; the complete XCTest suite passed 63/63. A signed build also installed on the connected iPhone, where first launch still waits for the free developer profile to be trusted in Settings.
...
- Remaining App Store gaps: deploy the deletion-preserving migration after client integration, authenticated two-account/device smoke, real-device permissions, Apple Developer Team/signing, App Store Connect app record, screenshots, native demo video, TestFlight upload, and final release review.
